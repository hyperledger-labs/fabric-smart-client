...
- **Bounded handler concurrency**: at most `handlerWorkers` (default 16) callbacks run concurrently, buffered by a queue of `handlerQueueSize` (default 1000) so bursts larger than the limit are still delivered in full. `handlerTimeout` (default 5s) only cancels the callback's context — it cannot force a return, so a callback that ignores cancellation occupies its slot indefinitely. Once every slot is occupied and the queue fills, the remaining listeners stay registered with the status the committer sent and are retried on the next sweep, so delivery is delayed rather than lost. This bounds the blast radius of a misbehaving listener to throughput rather than memory; see [notification service tuning](configuration.md#notification-service-tuning).
