...
The fastest way to validate a metric family is to scrape the FSC node directly:

```bash
curl http://<fsc-web-address>/metrics
```

If TLS is enabled on the FSC web endpoint, use the corresponding `https://` URL and client certificate settings required by the deployment.

FSC's web client also exposes a helper that parses the endpoint into Prometheus metric families:

- `platform/view/services/web/client.Client.Metrics()`

### Prometheus End-to-End

The repository contains a monitoring topology that provisions Prometheus and Grafana for integration environments:

- `integration/nwo/monitoring`

The existing Prometheus helper is:

- `integration/reporting/prometheus`

### Existing Examples

The repository already contains the following monitoring-related integration scenarios:

- **IOU integration**: enables FSC metrics and the Prometheus/Grafana monitoring topology and validates `fsc_view_services_view_calls_operations`
  - `integration/fabric/iou/topology.go`
  - `integration/fabric/iou/commands.go`
  - `integration/fabric/iou/iou_test.go`
- **Stop/Restart FSC integration**: enables FSC metrics and validates that the node exposes a parseable `/metrics` endpoint
  - `integration/fsc/stoprestart/topology.go`
  - `integration/fsc/stoprestart/stoprestart_test.go`

The repository also contains targeted tests for pieces of the monitoring stack itself, for example:

- the Prometheus and disabled metric providers
- the gRPC connection counters
- the `/metrics` parser used by the FSC web client
- the comm-layer dropped-message counter

These tests validate important parts of the monitoring surface, but they do not provide a complete end-to-end scenario for every metric family listed in this catalog.

Representative test locations are:

- `platform/view/services/metrics/prometheus/provider_test.go`
- `platform/view/services/metrics/disabled/provider_test.go`
- `platform/view/services/grpc/serverstatshandler_test.go`
- `platform/view/services/web/client/client_test.go`
- `platform/view/services/comm/stress_test.go`

### Runtime Validation Matrix

The table below maps the metric groups to concrete validation paths. No single scenario exercises the whole catalog.

| Metric groups | Example or topology | Trigger | Expected result |
| --- | --- | --- | --- |
| FSC node bootstrap and common view-service metrics | `integration/fsc/stoprestart` | invoke `init` once over gRPC and once over the web endpoint | validates node export of `fsc_view_services_metrics_operations_fsc_version`, `fsc_view_services_view_contexts`, `fsc_view_services_view_calls_*`, `fsc_view_services_view_grpc_server_requests_*`, and `fsc_view_services_view_web_view_client_*` |
| Comm and libp2p transport metrics | `integration/fsc/stoprestart` with `fsc.LibP2P` | run the `init` flow so Alice opens a session to Bob | validates `fsc_view_services_comm_*` on the happy path and `fsc_view_services_comm_host_libp2p_bytes_sent/received`; the normal flow does not exercise `fsc_view_services_comm_dropped_messages` |
| Comm and WebSocket transport metrics | `integration/fsc/stoprestart` with `fsc.WebSocket` | run the `init` flow so Alice reaches Bob over WebSocket transport | validates `fsc_view_services_comm_*`, `fsc_view_services_comm_host_websocket_ws_opened_websockets`, `fsc_view_services_comm_host_websocket_ws_opened_subconns`, `fsc_view_services_comm_host_websocket_ws_active_subconns`, `fsc_view_services_comm_host_websocket_ws_multiplexed_ws_operations`, and `fsc_view_services_comm_host_websocket_ws_multiplexed_ws_duration`; `fsc_view_services_comm_host_websocket_ws_closed_subconns` remains idle because no increment site exists in the current code |
| Caller-process gRPC client tracer metrics | custom caller process or dedicated client-side test | invoke a view through `platform/view/services/view/grpc/client` with a tracing provider created through `tracing.NewProviderWithBackingProvider(...)` | validates `fsc_view_services_view_grpc_client_client_*`; these families do not appear on the remote FSC node's `/metrics` |
| In-process local client tracer metrics | node code path that uses `platform/view/services/view/grpc/client.LocalClient` | invoke a view through the local client helper | validates `fsc_view_services_view_grpc_client_node_view_client_*`; this is a node-internal path, not the standard external ingress path |
| Fabric transaction, ordering, committer, delivery, and vault metrics | `integration/fabric/iou` | initialize approvers, create an IOU, update it, and wait for finality | exercises `fsc_fabric_core_generic_ordering_ordered_transactions`, the `fsc_fabric_core_generic_committer_*` families, `fsc_common_core_generic_vault_*`, `fsc_fabric_core_generic_delivery_delivery_*`, `fsc_common_core_generic_committer_finality_manager_*`, and `fsc_common_core_generic_committer_finality_listener_manager_*` |
| Prometheus scrape path | `integration/fabric/iou` with monitoring topology enabled | run the IOU flow and query Prometheus | exercises the scrape path for node-exported families; the current in-repo example already checks `fsc_view_services_view_calls_operations` |
| Targeted fault or edge-case metrics | dedicated unit or stress tests | force the specific failure mode directly | validates families that are not expected to move in happy-path runs, such as `fsc_view_services_comm_dropped_messages` and the custom gRPC connection counters |

### Validation Prerequisites

The Fabric transaction scenarios require more than the FSC-only scenarios:

- the local Fabric host binaries must be available through `FAB_BINS`
- the chaincode container runtime expected by the Fabric peer must be available through a Docker-compatible endpoint
