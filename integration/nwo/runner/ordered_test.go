...
var _ = Describe("Ordered Group", func() {
	...
	Describe("Start", func() {
		BeforeEach(func() {
			...
			members = grouper.Members{
				{Name: "child1", Runner: childRunner1},
				{Name: "child2", Runner: childRunner2},
			...
			Describe("when a process exits cleanly", func() {
				...
				Describe("when one of the processes exits with an error", func() {
					...
					It("returns an error indicating which child processes failed", func() {
						...
						Ω(errTrace).Should(ContainElement(grouper.ExitEvent{Member: grouper.Member{Name: "child1", Runner: childRunner1}}))
						Ω(errTrace).Should(ContainElement(grouper.ExitEvent{Member: grouper.Member{Name: "child2", Runner: childRunner2}, Err: errors.New("Fail")}))
					})
				})
			})
		})
	...
	Describe("Stop", func() {
		...
		Context("when runner receives a single signal", func() {
			BeforeEach(func() {
				...
				members = grouper.Members{
					{Name: "child1", Runner: r1},
					{Name: "child2", Runner: r2},
					{Name: "child3", Runner: r3},
				}
			})
