...
//
// Topology — three FSC nodes across three Fabric organisations:
//
//	+-----------+-----+--------------------------------------------------+
//	| node      | org | role                                             |
//	+-----------+-----+--------------------------------------------------+
//	| endorser  | Org1| initiator for InitLedger/CreateAsset and        |
//	|           |     | responder for every state-changing tx            |
...
// receiver still signs (as receiver-acceptance) — that signature is
// enforced by the initiator's CollectEndorsementsView, not by the
// namespace policy.
...
func Topology(sdk node.SDK, commType fsc.P2PCommunicationType) []api.Topology {
	...
	// validation logic and also initiates InitLedger/CreateAsset.
