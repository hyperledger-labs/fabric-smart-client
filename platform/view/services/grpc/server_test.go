...
func TestNewGRPCServerInvalidParameters(t *testing.T) {
	...
	require.Error(t, err, "[%s], [%s] [%s] or [%s] expected", msgs[0], msgs[1], msgs[2], msgs[3])
...
func TestNewGRPCServer(t *testing.T) {
	...
	assert.False(t, srv.MutualTLSRequired())
...
func TestNewSecureGRPCServer(t *testing.T) {
	...
	waitServerReady(t, testAddress, grpc.WithTransportCredentials(creds))
...
func TestVerifyCertificateCallback(t *testing.T) {
	...
	gRPCServer, err := grpc3.NewGRPCServer("127.0.0.1:", grpc3.ServerConfig{
		HealthCheckEnabled: true,
		SecOpts: grpc3.SecureOptions{
...
func runMutualAuth(t *testing.T, servers []testServer, trustedClients, unTrustedClients []*tls.Config) error {
	...
	for i := 0; i < len(servers); i++ {
		...
		// create GRPCServer
		srv, err := grpc3.NewGRPCServerFromListener(lis, servers[i].config)
		if err != nil {
			return err
		}

		// MutualTLSRequired should be true
		if !srv.MutualTLSRequired() {
			return errors.New("mTLS must be used")
		}
...
func TestSetClientRootCAs(t *testing.T) {
	...
	// Org1
	clientConfigOrg1 := testOrgs[0].trustedClients([][]byte{})[0]

	// should be ready
	waitServerReady(t, address, grpc.WithTransportCredentials(credentials.NewTLS(clientConfigOrg1)))
...
func TestServerInterceptors(t *testing.T) {
	...
	srvConfig := grpc3.ServerConfig{HealthCheckEnabled: true}
