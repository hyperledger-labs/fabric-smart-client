...
func NewKeysetPagination(offset int, pageSize int, sqlIdName common2.FieldName, idFieldName pagination.PropertyName) driver.Pagination {
	keysetPagination, err := pagination.Keyset[string](offset, pageSize, sqlIdName, idFieldName)
	if err != nil {
		Expect(err).ToNot(HaveOccurred())
	}
