...
func NewHostProvider(config driver.ConfigService, endpointService *endpoint.Service, metricsProvider metrics.Provider) (host.GeneratorProvider, error) {
	...
	if p2pCommType := config.GetString("fsc.p2p.type"); strings.EqualFold(p2pCommType, fsc.WebSocket) {
		return newWebSocketHostProvider(config, endpointService)
	} else {
...
func newWebSocketHostProvider(config driver.ConfigService, endpointService *endpoint.Service) (host.GeneratorProvider, error) {
	...
	return rest.NewEndpointBasedProvider(endpointService, discovery), nil
}
