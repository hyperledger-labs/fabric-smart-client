...
func (p *hostGeneratorProvider) GetNewHost() (host2.P2PHost, error) {
	...
	pk, err := newCryptoPublicKeyFromMSP(p.config.PublicKeyPath())
	if err != nil {
		return nil, err
	}
	if p.config.Bootstrap() {
		host, err := newLibP2PHost(p.config, pk, k, p.metrics, true, "")
		if err != nil {
			return nil, err
		}
	...
	host, err := newLibP2PHost(p.config, pk, k, p.metrics, false, bootstrap)
	if err != nil {
		return nil, err
	}
