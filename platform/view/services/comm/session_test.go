...
func setup() *NetworkStreamSession {
	...
	return newNetworkStreamSession(sessionID, contextID, endpointAddress, endpointID, callerViewID, caller, net)
}
...
func TestSessionLifecycleConcurrent(t *testing.T) {
	...
	go func() {
		...
		for i := range numMessage {
			s.received(&view.Message{Payload: []byte(fmt.Sprintf("msg #%v", i))})
		}
