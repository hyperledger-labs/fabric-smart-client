...
func BenchmarkTestLibp2pSession(b *testing.B) {
	...
	for i := range numOfConn {
		...
		sender.Start(context.Background())
		receiver, receiverPkID := NewLibP2PCommService(receiverConfig.Libp2pConfig(fmt.Sprintf("Sender%d", i)), receiverConfig.CertFile, &BootstrapNodeResolver{nodeID: senderPkID, nodeAddress: senderConfig.ListenAddress})
		receiver.Start(context.Background())
