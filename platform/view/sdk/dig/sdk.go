...
func NewSDKFrom(baseSDK dig2.SDK, registry node.Registry) *SDK {
	...
	err := errors.Join(
		...
		sdk.Container().Provide(digutils.Identity[*view.ConfigService](), dig.As(new(driver.ConfigService), new(id.ConfigProvider), new(endpoint2.ConfigService), new(dbdriver.Config))),
...
func (p *SDK) Install() error {
	err := errors.Join(
		...
		p.Container().Provide(endpoint2.NewService),
		...
		p.Container().Provide(digutils.Identity[*endpoint2.Service](), dig.As(new(driver.EndpointService))),
	...
	if err := p.Container().Invoke(func(resolverService *endpoint2.ResolverService) error { return resolverService.LoadResolvers() }); err != nil {
		return err
	}
	return nil
}
