...
func TestCheckTLSConfigRejectsRemovedKey(t *testing.T) {
	t.Parallel()
	// A removed key that lives OUTSIDE any tls: block, so only the explicit check can catch
	// it. The renamed keys *inside* a tls: block are rejected by the strict subtree decode
	// instead, and need no table entry — see tlsconfig/removed.go.
	p := providerFrom(t, `
fsc:
  metrics:
    prometheus:
