...
import (
	...
	"github.com/hyperledger-labs/fabric-smart-client/platform/fabric/msp/mocks"
...
func TestDeserializeIdentity(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = wrappedMSP.DeserializeIdentity(serializedIdentity2)
	require.NoError(t, err)
...
func TestSatisfiesPrincipal(t *testing.T) {
	...
	err = mockMSP.SatisfiesPrincipal(mockIdentity, mockMSPPrincipal)
	require.NoError(t, err)
