...
func CreateSignedTx(
	...
	var a1 []byte
	for n, r := range resps {
		if status := r.GetResponse().GetStatus(); status < 200 || status >= 400 {
			return nil, errors.Errorf("proposal response was not successful, error code %d, msg %s", status, r.GetResponse().GetMessage())
		}
