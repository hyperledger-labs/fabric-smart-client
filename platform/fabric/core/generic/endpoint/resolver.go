...
func (r *ResolverService) LoadResolvers() error {
	...
	for _, cfgResolver := range cfgResolvers {
		...
		// Add Resolver
		rootID, err := r.service.AddResolver(resolver.Ctx, resolver.Name, resolver.Domain, resolver.Addresses, resolver.Aliases, resolver.Id)
		if err != nil {
		...
		for _, alias := range resolver.Aliases {
			...
			if err := r.service.Bind(resolver.Ctx, resolver.Id, []byte(alias)); err != nil {
