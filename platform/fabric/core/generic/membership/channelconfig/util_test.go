...
func createCfgBlockWithSupportedCapabilities(t *testing.T) *cb.Block {
	...
	config.ChannelGroup.Groups[ApplicationGroupKey] = newConfigGroup()
	...
	config.ChannelGroup.Groups[OrdererGroupKey] = newConfigGroup()
...
func TestExtractMSPIDsForApplicationOrgs(t *testing.T) {
	...
	require.NoError(t, err)
	block := &cb.Block{}
	err = protolator.DeepUnmarshalJSON(bytes.NewBuffer(blockData), block)
	require.NoError(t, err)
