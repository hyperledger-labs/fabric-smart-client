...
func (c *Credential) Sign(key handlers.IssuerSecretKey, credentialRequest []byte, attributes []csp.IdemixAttribute) (res []byte, err error) {
	defer func() {
		if r := recover(); r != nil {
			res = nil
	...
	cred, err := crypto.NewCredential(iisk.SK, cr, attrValues, c.NewRand())
	if err != nil {
...
func (*Credential) Verify(sk handlers.Big, ipk handlers.IssuerPublicKey, credential []byte, attributes []csp.IdemixAttribute) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = errors.Errorf("failure [%s]", r)
		}
	...
	for i := 0; i < len(attributes); i++ {
		...
		case csp.IdemixBytesAttribute:
			if !bytes.Equal(
				crypto.BigToBytes(crypto.HashModOrder(attributes[i].Value.([]byte))),
				cred.Attrs[i]) {
				return errors.Errorf("credential does not contain the correct attribute value at position [%d]", i)
			}
		case csp.IdemixIntAttribute:
			if !bytes.Equal(
				crypto.BigToBytes(FP256BN.NewBIGint(attributes[i].Value.(int))),
				cred.Attrs[i]) {
				return errors.Errorf("credential does not contain the correct attribute value at position [%d]", i)
			}
