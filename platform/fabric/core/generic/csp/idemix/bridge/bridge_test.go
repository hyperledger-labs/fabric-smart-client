...
import (
	...
	"github.com/hyperledger-labs/fabric-smart-client/platform/fabric/core/generic/csp/idemix/handlers/mock"
...
var _ = Describe("Idemix Bridge", func() {
	...
	Describe("credential", func() {
		...
		Context("sign", func() {
			...
			It("fail on invalid attributes", func() {
				raw, err := Credential.Sign(issuerSecretKey, nil, []csp.IdemixAttribute{
					{Type: 5, Value: nil},
				})
		...
		Context("verify", func() {
			...
			It("fail on invalid attributes", func() {
				err := Credential.Verify(userSecretKey, issuerPublicKey, nil, []csp.IdemixAttribute{
					{Type: 5, Value: nil},
				})
				Expect(err).To(MatchError("attribute type not allowed or supported [5] at position [0]"))
			})
	...
	Describe("signature", func() {
		...
		Context("sign", func() {
			It("fail on nil user secret key", func() {
				signature, _, err := SignatureScheme.Sign(nil, nil, nil, nil, nil, nil, nil, 0, nil)
				Expect(err).To(MatchError("invalid user secret key, expected *Big, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil nym public key", func() {
				signature, _, err := SignatureScheme.Sign(nil, userSecretKey, nil, nil, nil, nil, nil, 0, nil)
				Expect(err).To(MatchError("invalid nym public key, expected *Ecp, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil nym secret key", func() {
				signature, _, err := SignatureScheme.Sign(nil, userSecretKey, nymPublicKey, nil, nil, nil, nil, 0, nil)
				Expect(err).To(MatchError("invalid nym secret key, expected *Big, got [<nil>]"))
				Expect(signature).To(BeNil())
			})

			It("fail on nil issuer public key", func() {
				signature, _, err := SignatureScheme.Sign(nil, userSecretKey, nymPublicKey, nymSecretKey, nil, nil, nil, 0, nil)
				Expect(err).To(MatchError("invalid issuer public key, expected *IssuerPublicKey, got [<nil>]"))
				Expect(signature).To(BeNil())
			})
		})
		...
		Context("verify", func() {
			...
			It("fail on invalid attributes", func() {
				err := SignatureScheme.Verify(issuerPublicKey, nil, nil,
					[]csp.IdemixAttribute{{Type: -1}}, 0, nil, 0)
	...
	Describe("setting up the environment with one issuer and one user", func() {
		...
		BeforeEach(func() {
			...
			IssuerKey, err = IssuerKeyGen.KeyGen(&csp.IdemixIssuerKeyGenOpts{Temporary: true, AttributeNames: AttributeNames})
			Expect(err).NotTo(HaveOccurred())
			...
			UserKey, err = UserKeyGen.KeyGen(&csp.IdemixUserSecretKeyGenOpts{})
			Expect(err).NotTo(HaveOccurred())

			// User Nym Key
			NymKeyDerivation = &handlers.NymKeyDerivation{User: User}
			NymKey, err = NymKeyDerivation.KeyDeriv(UserKey, &csp.IdemixNymKeyDerivationOpts{IssuerPK: IssuerPublicKey})
			Expect(err).NotTo(HaveOccurred())
			...
			RevocationKey, err = RevocationKeyGen.KeyGen(&csp.IdemixRevocationKeyGenOpts{})
			Expect(err).NotTo(HaveOccurred())
		...
		Context("the environment is not valid with the respect to different parameters", func() {
			...
			It("invalid credential request nonce", func() {
				valid, err := CredentialRequestVerifier.Verify(
					...
					credRequest,
					nil,
					&csp.IdemixCredentialRequestSignerOpts{IssuerNonce: []byte("pine-apple-pine-apple-pine-apple")},
				)
			...
			It("invalid credential request nonce, too short", func() {
				valid, err := CredentialRequestVerifier.Verify(
					...
					credRequest,
					nil,
					&csp.IdemixCredentialRequestSignerOpts{IssuerNonce: []byte("pin-apple-pine-apple-pineapple")},
				)
			...
			It("nil credential", func() {
				...
				valid, err := CredentialVerifier.Verify(
					...
					&csp.IdemixCredentialSignerOpts{
						IssuerPK: IssuerPublicKey,
						Attributes: []csp.IdemixAttribute{
							{Type: csp.IdemixBytesAttribute, Value: []byte{1}},
							{Type: csp.IdemixBytesAttribute, Value: []byte{0, 1}},
							{Type: csp.IdemixIntAttribute, Value: 1},
							{Type: csp.IdemixBytesAttribute, Value: []byte{0, 1, 2}},
							{Type: csp.IdemixHiddenAttribute},
				...
				valid, err := CredentialVerifier.Verify(
					...
					&csp.IdemixCredentialSignerOpts{
						...
						Attributes: []csp.IdemixAttribute{
							{Type: csp.IdemixBytesAttribute, Value: []byte{0}},
							{Type: csp.IdemixBytesAttribute, Value: []byte{0, 1}},
							{Type: csp.IdemixIntAttribute, Value: 2},
							{Type: csp.IdemixBytesAttribute, Value: []byte{0, 1, 2}},
							{Type: csp.IdemixHiddenAttribute},
						},
					},
		...
		Describe("producing and verifying idemix signature with different sets of attributes", func() {
			var (
				...
				SignAttributes   []csp.IdemixAttribute
				VerifyAttributes []csp.IdemixAttribute
				RhIndex          int
				Epoch            int
				errMessage       string
				validity         bool
			...
			It("the signature with different disclosed attributes is not valid", func() {
				...
				VerifyAttributes = []csp.IdemixAttribute{
					{Type: csp.IdemixBytesAttribute, Value: []byte{0}},
					{Type: csp.IdemixHiddenAttribute},
					{Type: csp.IdemixIntAttribute, Value: 10},
					{Type: csp.IdemixHiddenAttribute},
					{Type: csp.IdemixHiddenAttribute},
				}
			})
		...
		Describe("producing an idemix nym signature", func() {
			...
			Context("the signature is malformed", func() {
				var nymSignature *crypto.NymSignature

				BeforeEach(func() {
					nymSignature = &crypto.NymSignature{}
					err := proto.Unmarshal(signature, nymSignature)
					Expect(err).NotTo(HaveOccurred())
				})

				marshalAndVerify := func(nymSignature *crypto.NymSignature) (bool, error) {
