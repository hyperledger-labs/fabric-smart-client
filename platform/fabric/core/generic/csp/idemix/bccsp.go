...
type idemixCSP struct {
	*sw.CSP
}

func New(keyStore bccsp.KeyStore) (*idemixCSP, error) {
	base, err := sw.New(keyStore)
	if err != nil {
		return nil, errors.Wrap(err, "failed instantiating base csp")
	}

	idemixCSP := &idemixCSP{CSP: base}

	// key generators
	base.AddWrapper(reflect.TypeOf(&csp.IdemixIssuerKeyGenOpts{}), &handlers.IssuerKeyGen{Issuer: &bridge.Issuer{NewRand: bridge.NewRandOrPanic}})
	base.AddWrapper(reflect.TypeOf(&csp.IdemixUserSecretKeyGenOpts{}), &handlers.UserKeyGen{User: &bridge.User{NewRand: bridge.NewRandOrPanic}})
	base.AddWrapper(reflect.TypeOf(&csp.IdemixRevocationKeyGenOpts{}), &handlers.RevocationKeyGen{Revocation: &bridge.Revocation{}})
	...
	// importers
	base.AddWrapper(reflect.TypeOf(&csp.IdemixUserSecretKeyImportOpts{}), &handlers.UserKeyImporter{
		User: &bridge.User{},
	})
	base.AddWrapper(reflect.TypeOf(&csp.IdemixIssuerPublicKeyImportOpts{}), &handlers.IssuerPublicKeyImporter{
		Issuer: &bridge.Issuer{},
	})
	base.AddWrapper(reflect.TypeOf(&csp.IdemixNymPublicKeyImportOpts{}), &handlers.NymPublicKeyImporter{
		User: &bridge.User{},
	})
	base.AddWrapper(reflect.TypeOf(&csp.IdemixRevocationPublicKeyImportOpts{}), &handlers.RevocationPublicKeyImporter{})

	return idemixCSP, nil
}
...
func (csp *idemixCSP) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	// Validate arguments
	if k == nil {
...
// Notice that this is overriding the Sign methods of the sw impl. to avoid the digest check.
func (csp *idemixCSP) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	// Validate arguments
	if k == nil {
...
func (s *userSecreKeySignerMultiplexer) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	switch opts.(type) {
	case *csp.IdemixSignerOpts:
		return s.signer.Sign(k, digest, opts)
	case *csp.IdemixNymSignerOpts:
		return s.nymSigner.Sign(k, digest, opts)
	case *csp.IdemixCredentialRequestSignerOpts:
		return s.credentialRequestSigner.Sign(k, digest, opts)
	default:
		return nil, errors.New("invalid opts, expected *csp.IdemixSignerOpt or *csp.IdemixNymSignerOpts or *csp.IdemixCredentialRequestSignerOpts")
	}
}
...
func (v *issuerPublicKeyVerifierMultiplexer) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	...
	default:
		return false, errors.New("invalid opts, expected *csp.IdemixSignerOpts or *csp.IdemixCredentialRequestSignerOpts")
	}
}
