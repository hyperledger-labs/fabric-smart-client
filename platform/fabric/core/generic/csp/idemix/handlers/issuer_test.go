	...
	Describe("when creating an issuer key-pair", func() {
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			Context("and the secret key is exportable", func() {
				...
				It("returns no error and a key", func() {
					var err error
					sk, err = IssuerKeyGen.KeyGen(&csp.IdemixIssuerKeyGenOpts{})
					Expect(err).NotTo(HaveOccurred())
		...
		Context("and the underlying cryptographic algorithm fails", func() {
			...
			It("returns an error", func() {
				keyPair, err := IssuerKeyGen.KeyGen(&csp.IdemixIssuerKeyGenOpts{})
		...
		Context("and the options are not well formed", func() {
			...
			Context("and the option is nil", func() {
				It("returns error", func() {
					...
					Expect(err).To(MatchError("invalid options, expected *csp.IdemixIssuerKeyGenOpts"))
					Expect(sk).To(BeNil())
				})
			})

			Context("and the option is not of type *csp.IdemixIssuerKeyGenOpts", func() {
				It("returns error", func() {
	...
	Describe("when importing an issuer public key", func() {
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			Context("and the secret key is exportable", func() {
				...
				It("returns no error and a key", func() {
					var err error
					pk, err = IssuerPublicKeyImporter.KeyImport(fakeRaw, &csp.IdemixIssuerPublicKeyImportOpts{})
					Expect(err).NotTo(HaveOccurred())
		...
		Context("and the underlying cryptographic algorithm fails", func() {
			...
			It("returns an error", func() {
				pk, err := IssuerPublicKeyImporter.KeyImport([]byte{1, 2, 3}, &csp.IdemixIssuerPublicKeyImportOpts{})
				Expect(err).To(MatchError("new-key error"))
				Expect(pk).To(BeNil())
			})
		})
		...
		Context("and the arguments are not well formed", func() {
			...
			Context("and the raw is nil", func() {
				It("returns error", func() {
					pk, err := IssuerPublicKeyImporter.KeyImport(nil, &csp.IdemixIssuerPublicKeyImportOpts{})
			...
			Context("and the raw is empty", func() {
				It("returns error", func() {
					pk, err := IssuerPublicKeyImporter.KeyImport([]byte{}, &csp.IdemixIssuerPublicKeyImportOpts{})
			...
			Context("and the option is nil", func() {
				It("returns error", func() {
					...
					Expect(err).To(MatchError("invalid options, expected *csp.IdemixIssuerPublicKeyImportOpts"))
					Expect(pk).To(BeNil())
				})
			})

			Context("and the option is not of type *csp.IdemixIssuerPublicKeyImportOpts", func() {
				It("returns error", func() {
					pk, err := IssuerPublicKeyImporter.KeyImport([]byte{1, 2, 3}, &csp.IdemixNymPublicKeyImportOpts{})
					Expect(err).To(MatchError("invalid options, expected *csp.IdemixIssuerPublicKeyImportOpts"))
					Expect(pk).To(BeNil())
				})
			})
