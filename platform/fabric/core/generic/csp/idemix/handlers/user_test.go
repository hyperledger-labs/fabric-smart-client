	...
	Describe("when creating a user key", func() {
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			Context("and the secret key is exportable", func() {
				...
				It("returns no error and a key", func() {
					var err error
					sk, err = UserKeyGen.KeyGen(&csp.IdemixUserSecretKeyGenOpts{})
		...
		Context("and the underlying cryptographic algorithm fails", func() {
			...
			It("returns an error", func() {
				keyPair, err := UserKeyGen.KeyGen(&csp.IdemixUserSecretKeyGenOpts{})
	...
	Describe("when deriving a new pseudonym", func() {
		...
		Context("and the underlying cryptographic algorithm succeed", func() {
			...
			Context("and the secret key is exportable", func() {
				...
				It("returns no error and a key", func() {
					var err error
					nym, err = NymKeyDerivation.KeyDeriv(fakeUserSecretKey, &csp.IdemixNymKeyDerivationOpts{IssuerPK: fakeIssuerPublicKey})
					Expect(err).NotTo(HaveOccurred())
		...
		Context("and the options are not well formed", func() {
			...
			Context("and the user secret key is nil", func() {
				It("returns error", func() {
					nym, err := NymKeyDerivation.KeyDeriv(nil, &csp.IdemixNymKeyDerivationOpts{})
			...
			Context("and the user secret key is not of type *userSecretKey", func() {
				It("returns error", func() {
					nym, err := NymKeyDerivation.KeyDeriv(handlers.NewIssuerPublicKey(nil), &csp.IdemixNymKeyDerivationOpts{})
					Expect(err).To(MatchError("invalid key, expected *userSecretKey"))
					Expect(nym).To(BeNil())
				})
			})
			...
			Context("and the option is not of type *csp.IdemixNymKeyDerivationOpts", func() {
				BeforeEach(func() {
					fakeUserSecretKey = handlers.NewUserSecretKey(nil, false)
				})
			...
			Context("and the issuer public key is missing", func() {
				...
				It("returns error", func() {
					nym, err := NymKeyDerivation.KeyDeriv(fakeUserSecretKey, &csp.IdemixNymKeyDerivationOpts{})
			...
			Context("and the issuer public key is not of type *issuerPublicKey", func() {
				...
				It("returns error", func() {
					nym, err := NymKeyDerivation.KeyDeriv(fakeUserSecretKey, &csp.IdemixNymKeyDerivationOpts{IssuerPK: fakeUserSecretKey})
