...
type Credential interface {
	...
	Sign(key IssuerSecretKey, credentialRequest []byte, attributes []csp.IdemixAttribute) ([]byte, error)

	// Verify cryptographically verifies the credential by verifying the signature
	// on the attribute values and user's secret key
	Verify(sk Big, ipk IssuerPublicKey, credential []byte, attributes []csp.IdemixAttribute) error
}
...
type Revocation interface {
	...
	Sign(key *ecdsa.PrivateKey, unrevokedHandles [][]byte, epoch int, alg csp.RevocationAlgorithm) ([]byte, error)
	...
	Verify(pk *ecdsa.PublicKey, cri []byte, epoch int, alg csp.RevocationAlgorithm) error
}
...
type SignatureScheme interface {
	...
	// If attributes[i].Type == csp.IdemixHiddenAttribute then attribute i remains hidden and otherwise it is disclosed.
	// We require the revocation handle to remain undisclosed (i.e., attributes[rhIndex] == csp.IdemixHiddenAttribute).
	...
	Sign(cred []byte, sk Big, Nym Ecp, RNym Big, ipk IssuerPublicKey, attributes []csp.IdemixAttribute, msg []byte, rhIndex int, cri []byte) ([]byte, *csp.IdemixSignatureInfo, error)

	// Verify verifies an idemix signature.
	// The attribute slice steers which attributes it expects to be disclosed
	// If attributes[i].Type == csp.IdemixHiddenAttribute then attribute i remains hidden and otherwise
	...
	Verify(ipk IssuerPublicKey, signature, msg []byte, attributes []csp.IdemixAttribute, rhIndex int, revocationPublicKey *ecdsa.PublicKey, epoch int) error
}
