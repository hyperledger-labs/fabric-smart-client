...
func NewSignature(cred *Credential, sk *FP256BN.BIG, Nym *FP256BN.ECP, RNym *FP256BN.BIG, ipk *IssuerPublicKey, Disclosure []byte, msg []byte, rhIndex int, cri *CredentialRevocationInformation, rng *amcl.RAND) (*Signature, *csp.IdemixSignatureInfo, error) {
	// Validate inputs
	if cred == nil || sk == nil || Nym == nil || RNym == nil || ipk == nil || rng == nil || cri == nil {
		return nil, nil, errors.Errorf("cannot create idemix signature: received nil input")
	}

	if rhIndex < 0 || rhIndex >= len(ipk.AttributeNames) || len(Disclosure) != len(ipk.AttributeNames) {
		return nil, nil, errors.Errorf("cannot create idemix signature: received invalid input")
	}

	if cri.RevocationAlg != int32(ALG_NO_REVOCATION) && Disclosure[rhIndex] == 1 {
		return nil, nil, errors.Errorf("Attribute %d is disclosed but also used as revocation handle attribute, which should remain hidden.", rhIndex)
	}

	// locate the indices of the attributes to hide and sample randomness for them
	HiddenIndices := hiddenIndices(Disclosure)

	info := &csp.IdemixSignatureInfo{
		Disclosure: Disclosure,
		Attrs:      make([][]byte, len(cred.Attrs)),
		RAttrs:     make([][]byte, len(HiddenIndices)),
	}
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "failed to compute non-revoked proof")
	}
	...
	for i, j := range HiddenIndices {
		...
		// store decommitment in info
		info.Attrs[j] = cred.Attrs[j]
		rRaw := make([]byte, FP256BN.MODBYTES)
		rAttrs[i].ToBytes(rRaw)
		info.RAttrs[i] = rRaw
	}
	...
	return &Signature{
		...
		Epoch:              cri.Epoch,
		NonRevocationProof: nonRevokedProof}, info, nil
}
