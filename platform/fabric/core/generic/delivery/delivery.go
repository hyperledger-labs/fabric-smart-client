	...
	for {
		...
		default:
			if df == nil {
				...
				if err != nil {
					logger.Errorf("failed connecting to delivery service [%s:%s] [%s]. Wait 10 sec before reconnecting", d.network, d.channel, err)
					time.Sleep(10 * time.Second)
					if logger.IsEnabledFor(zapcore.DebugLevel) {
						logger.Debugf("reconnecting to delivery service [%s:%s]", d.network, d.channel)
					}
					continue
				}
			}
			...
			if err != nil {
				...
				logger.Errorf("delivery service [%s:%s:%s], failed receiving response [%s]",
					d.client.Address(), d.network, d.channel,
			...
			case *pb.DeliverResponse_Block:
				if r.Block == nil || r.Block.Data == nil || r.Block.Header == nil || r.Block.Metadata == nil {
					if logger.IsEnabledFor(zapcore.DebugLevel) {
						logger.Debugf("deliver service [%s:%s:%s], received nil block", d.client.Address(), d.network, d.channel)
					}
					time.Sleep(10 * time.Second)
					df = nil
				}
				...
				if logger.IsEnabledFor(zapcore.DebugLevel) {
					logger.Debugf("delivery service [%s:%s:%s], commit block [%d]", d.client.Address(), d.network, d.channel, r.Block.Header.Number)
				}
			...
			case *pb.DeliverResponse_Status:
				if r.Status == common.Status_NOT_FOUND {
					df = nil
					logger.Warnf("delivery service [%s:%s:%s] status [%s], wait a few seconds before retrying", d.client.Address(), d.network, d.channel, r.Status)
					time.Sleep(10 * time.Second)
				} else {
					logger.Warnf("delivery service [%s:%s:%s] status [%s]", d.client.Address(), d.network, d.channel, r.Status)
				}
			default:
				df = nil
				logger.Errorf("delivery service [%s:%s:%s], got [%s]", d.client.Address(), d.network, d.channel, r)
			}
		}
	}
}
...
func (d *Delivery) connect(ctx context.Context) (DeliverStream, error) {
	...
	if logger.IsEnabledFor(zapcore.DebugLevel) {
		logger.Debugf("connecting to deliver service at [%s] for [%s:%s]", address, d.network, d.channel)
	}
	ch, err := d.network.Channel(d.channel)
	if err != nil {
		return nil, errors.WithMessagef(err, "failed connecting to channel [%s:%s]", d.network, d.channel)
	}
	d.client, err = ch.NewPeerClientForAddress(*peerConnConf)
	if err != nil {
		return nil, errors.WithMessagef(err, "failed creating peer client for address [%s][%s:%s]", address, d.network, d.channel)
	}
