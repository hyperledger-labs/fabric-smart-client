...
func TTestInterceptorErr(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.EqualError(t, err, "invalid get option [15]")
	_, err = rws.GetState("foo", "bar", 15, 16)
	require.EqualError(t, err, "a single getoption is supported, 2 provided")
	...
	require.EqualError(t, err, "this instance was closed")
...
func TTestInterceptorConcurrency(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.NoError(t, err)
	require.Equal(t, []byte("val"), state.Raw)
}
...
func TTestInterceptorConcurrencyNoConsistency(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.EqualError(t, err, "invalid read [namespace:key1]: previous value returned at version [[]], current value at version [[0 0 0 35 0 0 0 1]]")
	...
	require.EqualError(t, err, "invalid metadata read: previous value returned at version [[]], current value at version [[0 0 0 36 0 0 0 1]]")
}
...
func TTestQueryExecutor(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.NoError(t, err)
	require.Equal(t, []byte("k1_value"), v.Raw)
	...
	require.NoError(t, err)
	require.Len(t, res, 4)
	require.ElementsMatch(t, []driver.VaultRead{
	...
	require.NoError(t, err)
	require.Len(t, res, 3)
	require.Equal(t, []driver.VaultRead{
		{Key: "k1", Raw: []byte("k1_value"), Version: versionBlockTxNumToBytes(35, 3)},
		{Key: "k111", Raw: []byte("k111_value"), Version: versionBlockTxNumToBytes(35, 4)},
		{Key: "k2", Raw: []byte("k2_value"), Version: versionBlockTxNumToBytes(35, 1)},
	}, res)
...
func TTestShardLikeCommit(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.NoError(t, rwsb.WriteSet.Add(ns, k1, []byte("k1FromTxidInvalid")))
	require.NoError(t, rwsb.WriteSet.Add(ns, k2, []byte("k2FromTxidInvalid")))
	...
	require.NoError(t, err)
	err = rwset.IsValid()
	require.EqualError(t, err, "invalid read: vault at version namespace:key2 [&{key2 [107 50 118 97 108] [0 0 0 37 0 0 0 3]}], read-write set at version [[0 0 0 37 0 0 0 2]]")
	...
	require.NoError(t, rwsb.WriteSet.Add(ns, k1, []byte("k1FromTxidValid")))
	require.NoError(t, rwsb.WriteSet.Add(ns, k2, []byte("k2FromTxidValid")))
	rwsBytes, err = marshaller.Marshal("pineapple", rwsb)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, &driver.VaultRead{Key: "key1", Raw: []byte("k1FromTxidValid"), Version: versionBlockTxNumToBytes(38, 10)}, vv)

	vv, err = ddb.GetState(context.Background(), ns, k2)
	require.NoError(t, err)
	require.Equal(t, &driver.VaultRead{Key: "key2", Raw: []byte("k2FromTxidValid"), Version: versionBlockTxNumToBytes(38, 10)}, vv)

	// all interceptors should be gone
	require.Empty(t, aVault.interceptors)
}
...
func TTestVaultErr(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.ErrorContains(t, err, "read-write set for txids [[non-existent]] could not be found")
	...
	require.EqualError(t, err, "programming error: rwset already exists for [not-closed]")
	_, err = vault1.NewRWSetFromBytes(context.Background(), "not-closed", rwsBytes)
	require.ErrorContains(t, err, "programming error: rwset already exists for [not-closed]")
	err = vault1.CommitTX(context.TODO(), "not-closed", 0, 0)
	require.ErrorContains(t, err, "attempted to retrieve read-write set for not-closed when done has not been called")
	err = vault1.DiscardTx(context.Background(), "not-closed", "")
	require.EqualError(t, err, "attempted to retrieve read-write set for not-closed when done has not been called")
	...
	require.NoError(t, err)
	require.Equal(t, "pineapple", message)
	require.Equal(t, invalid, vc)
...
func TTestMerge(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.NoError(t, rwsb.WriteSet.Add(ns, k1, []byte("newv1")))
	require.NoError(t, rwsb.MetaWriteSet.Add(ns, k1, map[string][]byte{"k1": []byte("v1")}))
	m := vp.NewMarshaller()
	...
	require.NoError(t, err)
	require.Equal(t, NamespaceKeyedMetaWrites{
		"namespace": {
			"key1": {"k1": []byte("v1")},
			"key3": {"k3": []byte("v3")},
		},
	}, rw.MetaWrites)
	require.Equal(t, Writes{"namespace": {
		"key1": []byte("newv1"),
		"key2": []byte("v2"),
	}}, rw.Writes)
	require.Equal(t, Reads{
		"namespace": {
	...
	err = rws.AppendRWSet(rwsBytes)
	require.EqualError(t, err, "duplicate write entry for key namespace:key2")
	...
	err = rws.AppendRWSet(rwsBytes)
	require.EqualError(t, err, "duplicate metadata write entry for key namespace:key3")
}
...
func TTestInspector(t *testing.T, ddb driver2.VaultStore, vp artifactsProvider) {
	...
	require.Equal(t, 1, i.NumReads(ns))
	require.Equal(t, 1, i.NumWrites(ns))
	require.Equal(t, []string{"ns"}, i.Namespaces())
...
func TTestRun(t *testing.T, db1, db2 driver2.VaultStore, vp artifactsProvider) {
	...
	require.NoError(t, err)
	require.Equal(t, map[string][]byte{"metakey": []byte("metavalue")}, vMap)
	...
	v, err = rws.GetState(ns, k1, driver.FromIntermediate)
	require.NoError(t, err)
	require.Equal(t, []byte("v1_updated"), v)
	...
	vMap, err = rws.GetStateMetadata(ns, k1Meta, driver.FromIntermediate)
	require.NoError(t, err)
	require.Equal(t, map[string][]byte{"newmetakey": []byte("newmetavalue")}, vMap)
	...
	v, err = rws.GetState(ns, k2, driver.FromIntermediate)
	require.NoError(t, err)
	require.Equal(t, []byte("v2_updated"), v)
	...
	require.NoError(t, err)
	require.NotNil(t, rwsBytes)

	require.NoError(t, vault1.Match(context.Background(), txid, rwsBytes))
	require.Error(t, vault1.Match(context.Background(), txid, []byte("pineapple")))

	// we open the read-write set fabric.From the other kvs
	rws, err = vault2.NewRWSetFromBytes(context.Background(), txid, rwsBytes)
	require.NoError(t, err)

	require.Equal(t, []string{ns}, rws.Namespaces())
	// we check reads positionally
	nReads := rws.NumReads(ns)
	require.Equal(t, 3, nReads)
	rKey, rKeyVal, err := rws.GetReadAt(ns, 0)
	require.NoError(t, err)
	require.Equal(t, k1, rKey)
	require.Equal(t, []byte("v1"), rKeyVal)
	rKey, rKeyVal, err = rws.GetReadAt(ns, 1)
	require.NoError(t, err)
	require.Equal(t, k1Meta, rKey)
	require.Empty(t, rKeyVal)
	rKey, rKeyVal, err = rws.GetReadAt(ns, 2)
	require.NoError(t, err)
	require.Equal(t, k2, rKey)
	require.Equal(t, []byte(nil), rKeyVal)
	_, _, err = rws.GetReadAt(ns, 3)
	require.EqualError(t, err, "no read at position 3 for namespace namespace")
	nReads = rws.NumReads("barf")
	require.Equal(t, 0, nReads)
	// we check writes positionally
	nWrites := rws.NumWrites(ns)
	require.Equal(t, 2, nWrites)
	nWrites = rws.NumWrites("barfobarfs")
	require.Equal(t, 0, nWrites)
	wKey, wKeyVal, err := rws.GetWriteAt(ns, 0)
	require.NoError(t, err)
	require.Equal(t, k1, wKey)
	require.Equal(t, []byte("v1_updated"), wKeyVal)
	wKey, wKeyVal, err = rws.GetWriteAt(ns, 1)
	require.NoError(t, err)
	require.Equal(t, k2, wKey)
	require.Equal(t, []byte("v2_updated"), wKeyVal)
	_, _, err = rws.GetWriteAt(ns, 2)
	require.EqualError(t, err, "no write at position 2 for namespace namespace")
	...
	require.NoError(t, err)
	require.Equal(t, k1, wKey)
	require.Equal(t, []byte(nil), wKeyVal)
	wKey, wKeyVal, err = rws.GetWriteAt(ns, 1)
	require.NoError(t, err)
	require.Equal(t, k2, wKey)
	require.Equal(t, []byte("v2_updated"), wKeyVal)
	...
	// all interceptors should be gone
	require.Empty(t, vault1.interceptors)
	require.Empty(t, vault2.interceptors)
	...
	require.NoError(t, err)
	require.Nil(t, vv1.Raw)
	require.Zero(t, vv1.Version)

	vv2, err := db2.GetState(context.Background(), ns, k1)
	require.NoError(t, err)
	require.Equal(t, vv1, vv2)

	vv1, err = db1.GetState(context.Background(), ns, k2)
	require.NoError(t, err)
	vv2, err = db2.GetState(context.Background(), ns, k2)
	require.NoError(t, err)
	require.Equal(t, &driver.VaultRead{Key: "key2", Raw: []byte("v2_updated"), Version: versionBlockTxNumToBytes(35, 2)}, vv1)
	require.Equal(t, vv1, vv2)
	...
	require.NoError(t, err)
	require.Equal(t, map[string][]byte{"newmetakey": []byte("newmetavalue")}, meta1)
	require.Equal(t, uint64(35), b1)
	require.Equal(t, uint64(2), t1)
